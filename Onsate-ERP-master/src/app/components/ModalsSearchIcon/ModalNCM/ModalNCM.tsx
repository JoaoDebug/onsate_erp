...
import styles from "./ModalNCM.module.css";
...
const ModalUnitiesSelect: React.FC<ModalProps> = ({
  ...
  const [initialNcms, setInitialNcms] = useState<ncmData[]>([]);
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(
          `http://26.56.52.76:8000/ncms?limit=${limitItemsPerPage}&page=${currentPage + 1}
          }`
        );
  ...
  const handleDoubleClick = (ncm: string) => {
    console.log(ncm, "Nome para o campo");
    onSelectUnity(ncm);
  ...
  const handleSelectLineBtn = () => {
    ...
    const ncmSelected = data.filter((item, index) => index === selected);

    handleDoubleClick(ncmSelected[0].ncm);
  };
  ...
  const filterData = async (searchTerm: string, field: string) => {
    try {
      const res = await fetch(
        ...
        `http://26.56.52.76:8000/ncms?${field}=${searchTerm}&limit=${limitItemsPerPage}}`
      );
                ...
                <tbody>
                  {data.map((item, index) => (
                    <tr
                      ...
                      onDoubleClick={() => handleDoubleClick(item.ncm)}
                    >
                      <td>{item.id}</td>
                      <td>{item.ncm}</td>
